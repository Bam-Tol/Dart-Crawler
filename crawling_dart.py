...
## 철회 통지
# http://dart.fss.or.kr/dsaf001/main.do?rcpNo=20180327000781
...
#
#     base_info_url = 'https://opendart.fss.or.kr/api/corpCode.xml'
#     params= {'crtfc_key': crtfc_key}
#     response = requests.get(base_info_url , params=params)
#
#     with ZipFile(BytesIO(response.content)) as zipfile:
#         zipfile.extractall('zipfile')
#
# # xml 호출하여 읽어오기
# xmlTree = parse('zipfile/CORPCODE.xml')
# root = xmlTree.getroot()
# my_list = root.findall('list')
# my_columns=['corp_code', 'corp_name', 'stock_code', 'modify_date']
# info_df = pd.DataFrame(columns=my_columns)
# for seq in range(len(my_list)):
#     code = my_list[seq].findtext('corp_code')
#     name = my_list[seq].findtext('corp_name')
#     stock = my_list[seq].findtext('stock_code')
#     modify = my_list[seq].findtext('modify_date')
#     info_df.loc[seq, my_columns] = [code, name, stock, modify]
#
#
# info_df.to_csv('stock_code.csv', encoding='cp949')
